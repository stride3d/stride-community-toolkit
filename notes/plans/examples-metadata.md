...
**Status: steps 0-6 done. Phases 1, 2 and §5 are all complete.** Every example carries schema v1
...
it. §5 automated screenshots is built, reviewed and committed: **55 images, 51 of the 70 example pages
now carry one**, up from 25 hand-taken images before. Three captures remain outstanding - see
§5 *Outstanding*.

---

## 1. Current state (re-verified 2026-08-27)
...
| Docs `.md` files under `examples/` | **70** - 41 generated, 29 hand-owned | 33 |
| Example pages carrying a screenshot | **51 of 70** | 25 |
| Examples with **no doc at all** | **1** - `stride-signalr-server`, deliberate (`docs: false`, D45) | ~29 |
| Docs that already have YAML frontmatter | **41** - generated pages carry `generated: true` and, for redirects, `redirect_url` | 0 |
...
## 5. Automated screenshots - **DONE 2026-08-27** (built 08-23, reviewed and committed 08-26)
...
### Review verdict - 53 judged, 49 accepted

The contact sheet was judged in one pass: **49 accepted, 4 rejected as "wrong moment"** - the capture
frame catches the scene before it settles, or the default camera is pointed somewhere unhelpful.
No image was rejected for framing or quality, which is the evidence that `IsFixedTimeStep` plus a fixed
frame count is a sound capture strategy; the failures are all *content* decisions belonging to the
example, not to the capture tool.

The accepted 49 were committed to `docs/manual/code-only/examples/media` - 20 replacing hand-taken
images, 28 new. Media count went 25 → 53, and 53 → 55 after the two follow-ups below.

### Outstanding

| Slug | Example | State |
|---|---|---|
| `constraint-rope` | `Example15_Constraint_Rope` | **Ready to recapture.** Anchor, lighting and camera adjusted in 857d61ce |
| `debug-shapes` | `Example08_DebugShapes` | Camera unchanged since the review - still needs a look |
| `instancing` | `Example21_Instancing` | Camera sits at `(0, 13, 115)`, far from the two walls it is meant to show. Wants a closer position, not a different frame |
| `stress-pile-2d` | `Example01_Basic2DScene_StressPile` | Pile still falling at frame 240. Needs a larger `screenshotFrame`, not a camera change |

Each is fixed either by editing the example's default camera or by setting `screenshotFrame` in its
frontmatter, then `--review --only <slug>` to rebuild the sheet and re-judge just that one.

### What the first full run found - four defects, none of them in the capture code

The bulk run doubles as an integration test of every example, which is most of its value. Running all
57 surfaced four separate problems that reading the code had not:

- **A duplicate `media:` filename silently destroyed an image.** 49 captures produced 48 files:
  `Example05_PartialTorus` and `Example05_PartialTorus_FSharp` both declared the same media name, so the
  second overwrote the first. Caught only by counting. Fixed in the data, and the class of bug is now
  impossible: `MetadataValidator.ValidateMediaUniqueness` errors on it (**D51**), and
  `capture-screenshots` refuses to start a docs-mode run whose resolved filenames collide.
- **`--media-path` had no default, so the docs generator linked no images at all.** 0 of 41 generated
  pages carried one, because the generator only links a media file it can actually see and nothing was
  telling it where to look. Giving the `docs` command a `DefaultValueFactory` took it to 26 immediately
  (**D52**). This was by far the largest of the four and had been live, unnoticed, since step 1.4.
- **Both ImGui integrations asserted under `IsFixedTimeStep`.** `NewFrame` in `Update` paired with
  `Render` in `EndDraw` assumes exactly one update per draw; a fixed timestep issues catch-up updates and
  the second `NewFrame` trips a native Dear ImGui assertion, which aborts rather than throws. Fixed in
  `ImGuiSystem` and `ImGuiNetSystem` with a `_frameBegun` guard (**D53**). This is a real bug for any
  consumer of the toolkit, not a screenshot artefact - the capture run is simply what exposed it.
- **MSBuild mangled the generator's output.** `ExamplesManifest.targets` set `StdErrEncoding="utf-8"`
  but not `StdOutEncoding`, so validation errors rendered correctly while ordinary output came through
  the OEM code page as `Ô£à` and `ÔÇö`.

### Aiming a camera is now a two-minute job (D54)

Three of the four rejects are camera problems, and fixing one previously meant editing, rebuilding and
guessing. Two additions close that loop:

- `Basic3DCameraController`'s F2 overlay prints the live `Position` and `Rotation (YPR)` in degrees. Fly
  to the shot, read the numbers.
- `game.SetCameraPosition(...)` / `game.SetCameraRotation(...)` apply them **after**
  `SetupBase3DScene()`, so an example does not have to unfold the base scene to move its camera.
  `Add3DCamera` cannot be reused for this - it builds a second entity bound to the same camera slot.

The overlay's numbers paste straight into the setters: verified numerically, since
`Quaternion.RotationYawPitchRoll` → `Quaternion.YawPitchRoll` round-trips to within 0.01° including the
near-gimbal case.
...
| D51 | Duplicate `media:` | **Error, not a warning** - unlike duplicate `order` (D46), which degrades gracefully. A media collision is silent data loss: the second capture overwrites the first and both pages then show one image. Only explicit values can collide, since the default is derived from the slug, which is already unique |
| D52 | `--media-path` default | **Required on `docs`, optional on `scan`/`generate`.** The asymmetry is deliberate: for scan and generate the path only enables an extra existence check, so absent means "skip a check". For docs it decides whether a page gets an image at all, so absent silently produced 41 image-less pages. Defaults to `../../docs/manual/code-only/examples/media` |
| D53 | ImGui frame pairing | Guard with a `_frameBegun` flag rather than reordering game systems or forcing `IsFixedTimeStep = false`. Earlier `DrawOrder` attempts failed because the problem was never draw ordering - a fixed timestep can issue several `Update`s per `Draw`, so `NewFrame` must close any frame it finds already open. Applies to both `Stride.CommunityToolkit.ImGui` and `.ImGuiNet` |
| D54 | Camera aiming API | Separate **read** (F2 overlay prints live position and YPR) from **write** (`SetCameraPosition` / `SetCameraRotation`). Not a new `Add3DCamera` overload: `Add3DCamera` creates an entity for camera slot 0, so calling it after `SetupBase3DScene` leaves two cameras contending for one slot. Rotation is degrees in the same X=Yaw, Y=Pitch, Z=Roll packing `Add3DCamera` already uses, so overlay output is copy-pasteable |
